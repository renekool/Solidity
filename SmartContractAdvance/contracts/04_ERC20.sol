...
interface IERC20 {
    ...
    // Determina la cantidad total de tokens existentes en la blockchain
    function totalSupply() external view returns (uint256);    

    // Calcula el balance de tokens en una cuenta específica
    ...
    /* Establece un `amount` como la asignacion de `spender` sobre los tokens 
...
abstract contract ERC20 is IERC20 {
    ...
    // Devuelve el simbolo del token, una version mas corta del nombre.
    ...
    // La funcion que anula esa funcion base debe marcarse como "override"
    ...
    /* Ver: {IERC20-transfer}.
    Permite la transferencia de tokens de una cuenta a otra
    Requisitos:
    ...
    // Permite devolver un value del spender de un owner    
    function allowance(address owner, address spender) public view virtual override returns (uint256) {
        return _allowances[owner][spender];        
    }

    // Permite la asignación del límite para gastar del spender del owner
    ...
    Permite la transferencia de tokens de una cuenta a otra con un máximo permitido del spender
    Emite un evento de {Approval} indicando el permiso actualizado. Esto no es requerido por la EIP. 
    Requisitos:
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        /*  • El bloque unchecked desactiva esta verificación automática para las operaciones 
            aritméticas dentro de su alcance. Esto puede ser útil en ciertos casos donde el 
            programador está seguro de que la operación no resultará en un desbordamiento y 
            quiere optimizar el consumo de gas al evitar las verificaciones adicionales.
            
            • El bloque unchecked en la función {decreaseAllowance} se utiliza para OPTIMIZAR 
            EL CONSUMO DE GAS porque se ha garantizado mediante una verificación previa que 
            la resta no causará un subdesbordamiento. */
        unchecked {
    ...
    implementar fees (tarifas) automaticas de tokens, etc.
    Emite un evento {Transfer}.
    Requisitos:
     • `from` y `to` no pueden ser direcciones cero.
     • `from` debe tener un saldo de al menos `amount`. */
    ...
    /* Crea tokens de `amount` y las asigna a `account`, aumentando el suministro total.
    ...
    /* Destruye la `amount` de tokens de `account`, reduciendo el suministro total.
    ...
    /* Hook que se llama ANTES de cualquier transferencia de tokens. Esto incluye "minting" y "burning".
    Condiciones de llamada:
     • Cuando `from` y `to` son ambos distintos de cero, `amount` de los tokens de `from` se transferira a `to` .
    ...
    /* Hook que se llama DESPUÉS de cualquier transferencia de tokens. Esto incluye "minting" y "burning".
    Condiciones de llamada:
     • Cuando `from` y `to` son ambos distintos de cero, `amount` de los tokens de `from` ha sido transferido a `to` .
